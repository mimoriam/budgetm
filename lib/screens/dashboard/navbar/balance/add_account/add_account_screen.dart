                ...
                if (_formKey.currentState?.saveAndValidate() ?? false) {
                  try {
                    ...
                    final balanceLimitValue = !_isCreditSelected
                        ? double.tryParse(values['transaction_limit']?.toString() ?? '')
                        : null;
                    // Parse transaction_limit explicitly so it can be saved separately
                    final transactionLimitValue = double.tryParse(values['transaction_limit']?.toString() ?? '');
                    ...
                    if (includeInTotal && inputAmount > 0) {
                      ...
                      final initialTransaction = FirestoreTransaction(
                        ...
                        amount: inputAmount.abs(), // Use absolute value of user-entered amount
